...
// Contacto — cada campo es corto (handle o número)
const MAX_CONTACT:     usize = 50;

const MAX_RESUBMITS:       u8  = 3;
const REVIEW_TIMEOUT_SECS: i64 = 7 * 24 * 3600;

#[derive(AnchorSerialize, AnchorDeserialize, Clone, PartialEq, Eq)]
pub enum TaskStatus {
    Open, InProgress, Submitted, Disputed, Paid, Cancelled,
}
...
pub mod blinktasks {
    ...
    pub fn init_profile(
        ...
        bio:       String,
        skills:    String,
        whatsapp:  String,   // ← nuevo: número o link wa.me
        discord:   String,   // ← nuevo: usuario#0000 o solo usuario
        telegram:  String,   // ← nuevo: @usuario
        github:    String,   // ← nuevo: usuario de GitHub
    ) -> Result<()> {
        ...
        require!(whatsapp.len() <= MAX_CONTACT,   CustomError::StringTooLong);
        require!(discord.len()  <= MAX_CONTACT,   CustomError::StringTooLong);
        require!(telegram.len() <= MAX_CONTACT,   CustomError::StringTooLong);
        require!(github.len()   <= MAX_CONTACT,   CustomError::StringTooLong);
    ...
    pub fn create_task(
        ...
        amount: u64, title: String, description: String,
        category: String, deadline: i64,
    ) -> Result<()> {
    ...
    pub fn submit_delivery(ctx: Context<SubmitDelivery>, delivery_url: String) -> Result<()> {
        ...
        let task = &mut ctx.accounts.task;
        require!(task.status == TaskStatus::InProgress, CustomError::InvalidStatus);
    ...
    /// Resubmit después de una disputa — lleva contador
    pub fn submit_delivery_retry(ctx: Context<SubmitDelivery>, delivery_url: String) -> Result<()> {
        require!(delivery_url.len() <= MAX_DELIVERY, CustomError::StringTooLong);
        let task = &mut ctx.accounts.task;
        require!(task.status == TaskStatus::Disputed,     CustomError::InvalidStatus);
        require!(task.worker == *ctx.accounts.worker.key, CustomError::Unauthorized);
        require!(task.resubmit_count < MAX_RESUBMITS,     CustomError::MaxResubmitsReached);

        task.delivery_url    = delivery_url;
        task.status          = TaskStatus::Submitted;
        task.error_note      = String::new();
        task.resubmit_count += 1;
        task.submitted_at    = Clock::get()?.unix_timestamp;
        Ok(())
    }
    ...
    /// Cliente reporta error — bloqueado si worker agotó resubmits
    ...
    /// Cliente cancela cuando el worker agotó sus 3 resubmits
    pub fn cancel_dispute(ctx: Context<CancelDispute>) -> Result<()> {
        let task = &mut ctx.accounts.task;
        require!(
            task.status == TaskStatus::Disputed || task.status == TaskStatus::Submitted,
            CustomError::InvalidStatus
        );
        require!(task.creator == *ctx.accounts.creator.key, CustomError::Unauthorized);
        require!(task.resubmit_count >= MAX_RESUBMITS,       CustomError::ResubmitsNotExhausted);
    ...
    /// Worker cobra si el cliente no responde en 7 días
    pub fn claim_after_timeout(ctx: Context<ClaimAfterTimeout>) -> Result<()> {
        ...
        require!(now >= task.submitted_at + REVIEW_TIMEOUT_SECS, CustomError::TimeoutNotReached);
    ...
    /// Cancela tarea Open, regresa SOL al creador
...
pub struct UserProfile {
    ...
    pub whatsapp:        String,   // 4+50  ← nuevo
    pub discord:         String,   // 4+50  ← nuevo
    pub telegram:        String,   // 4+50  ← nuevo
    pub github:          String,   // 4+50  ← nuevo
...
// 8 + 32 + 54 + 204 + 104 + 54+54+54+54 + 8*5 = 658
const PROFILE_SPACE: usize = 8 + 32 + 54 + 204 + 104 + 54 + 54 + 54 + 54 + 8 + 8 + 8 + 8 + 8;
...
// 8 + 32+32+8 + 104+504+54+8+204+204 + 2+1+8+1+1+8 = 1183
const TASK_SPACE: usize = 8 + 32 + 32 + 8 + 104 + 504 + 54 + 8 + 204 + 204 + 2 + 1 + 8 + 1 + 1 + 8;
...
pub struct InitProfile<'info> {
    #[account(init, payer = user, space = PROFILE_SPACE,
...
pub struct CreateTask<'info> {
    #[account(init, payer = creator, space = TASK_SPACE,
        seeds = [b"task", creator.key().as_ref(), &profile.tasks_created.to_le_bytes()], bump)]
...
pub enum CustomError {
    ...
    InvalidRating,
    #[msg("Worker has reached the maximum number of resubmits")]
    MaxResubmitsReached,
    #[msg("Resubmits are not exhausted yet")]
    ResubmitsNotExhausted,
    #[msg("Review timeout has not been reached yet (7 days required)")]
    TimeoutNotReached,
}